...
\begin{drawgraph}{ejes=-4:4 -4:4,scale=0.8}
\drawfunction{x^2}
\end{drawgraph}
\end{minipage}
\begin{minipage}[c]{.55\textwidth}
\begin{verbatim}
\begin{drawgraph}{ejes=-4:4 -4:4,%
scale=0.8}
\drawfunction{x^2}
\end{drawgraph}
%
...
\begin{tikzpicture}
\node [tn_red] {33}
	child{node [tn_black] {15}
		child{node [tn_black] {10}
            child{node [tn_black] {5}}
            child{node [tn_null] {}
			}
		}
		child {node [tn_null] {}}
		edge from parent node [above left] {left}
    }
	child{node [tn_red] {47}
		child{node [tn_red] {38}
			child{ node [tn_black] {36}}
			child{ node [tn_red] {39}}
		}
		child{node [tn_black] {51}
			child{ node [tn_black] {49}}
			child{ node [tn_null] {}}
		}
		edge from parent node [above right] {right}
	};
\end{tikzpicture}
\end{center}
...
o.callMethod(b ,c)\;
\While{o is not null}{
	o $\leftarrow$ myFunction(a,b)\;
}
\If{o is null}{
	print ''this is a pretty stupid check at this point''
}
\Return o
...
\gqm{Drüber/Drunter} & \verb|\ouset{middle}| & $\ouset{middle}{over}{under}$\\
                      & \verb|{over}{under}| &\\
\hline
...
\hline
\gqm{$\mr$-Vektorraum} & \verb|\rvecroom| & \rvecroom\\
\gqm{linear unabhängig} &\verb|\lu| & \lu\\
\gqm{linear abhängig} & \verb|\la| & \la\\
\gqm{lin. Gl.sys.} & \verb|\lgs| & \lgs\\
